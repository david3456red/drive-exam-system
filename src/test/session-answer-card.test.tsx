...
describe('AnswerCard', () => {
  it('renders numbered jump buttons with visual states and attempt id', () => {
    render(
      <AnswerCard
        ...
        items={[
          ...
          { number: 4, questionId: 'q4', state: 'answered', answered: true },
          { number: 5, questionId: 'q5', state: 'empty', answered: false },
        ]}
    ...
    expect(screen.getByRole('button', { name: '第 2 题，回答错误' })).toHaveClass(
      'answer-card-cell-wrong',
    );
    expect(screen.getByRole('button', { name: '第 3 题，当前试题' })).toHaveClass(
