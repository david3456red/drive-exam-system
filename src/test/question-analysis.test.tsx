...
describe('QuestionAnalysis', () => {
  it('reveals the answer and explanation on demand', () => {
    ...
    expect(screen.getByLabelText('题目解析')).toHaveTextContent('正确答案：C');
    expect(screen.getByLabelText('题目解析')).toHaveTextContent('文字说明：观察交警手势后选择变道信号。');
    expect(screen.getByRole('button', { name: '收起解析' })).toHaveAttribute('aria-expanded', 'true');
  });
  ...
  it('uses a fallback when the question has no explanation', () => {
    ...
    expect(screen.getByLabelText('题目解析')).toHaveTextContent('暂无解析');
  });
});
