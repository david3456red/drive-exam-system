import type { AnswerCardItem, AnswerCardState } from '@/lib/exam-engine/answer-card';
      ...
      <form action={action as string} aria-label="图形答题卡" className="answer-card-grid" role="group">
        ...
        {items.map((item) => (
          <button
            ...
            aria-label={`第 ${item.number} 题，${answerCardStateText(item.state)}`}
            className={`answer-card-cell answer-card-cell-${item.state}`}
            key={item.questionId}
            name="questionId"
            type="submit"
...
function answerCardStateText(state: AnswerCardState): string {
  switch (state) {
    case 'current':
      return '当前试题';
    case 'correct':
